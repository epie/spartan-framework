...
class GCloudLogger:
    ...
    - Proper severity levels (DEFAULT, DEBUG, INFO, NOTICE, WARNING, ERROR, CRITICAL, ALERT, EMERGENCY)
    ...
    def __init__(
        ...
        self.sample_rate = sample_rate if sample_rate is not None else float(
            env("LOG_SAMPLE_RATE", "1.0")
        )
        ...
        if self.client:
            ...
            self.logger = None
            self.use_gcp = False
            print(f"Warning: GCP Logging not available, using fallback for {service_name}")
    ...
    def _get_source_location(self) -> Dict[str, Any]:
        ...
        for frame_info in stack:
            ...
            if any(skip in normalized_path for skip in [
                "/services/logging/",
                "/helpers/logger.py",
                "/logging/",
                "site-packages/",
    ...
    def _write_log(
        ...
        if self.use_gcp and self.logger:
            ...
            import json
            log_entry = self._create_log_entry(severity, message, extra, exc_info)
            print(json.dumps(log_entry, default=str, indent=2))
